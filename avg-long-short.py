...
class Position(object):
    def __init__(self):
        ...
        self.last_price = 0
        self.side = 'long'
        self.long_next_entry = 0
        self.short_next_entry = 0
        self.short_exit = 0
        self.long_exit = 0
    ...
    def calc_prices(self):
        diff = 0
        try:
            diff = exp.index(self.qty/10) / 50
        except:
            for e in exp:
                if self.qty/initial_qty > e:
                    diff += 0.02

        if self.side == 'long':
            self.long_next_entry = self.last_price - target_price - diff
            self.long_exit = self.entry_price + target_price
        elif self.side == 'short':
            self.short_next_entry = self.last_price + target_price + diff
            self.short_exit = self.entry_price - target_price
        
        print('position -- ', vars(self))
...
class My(object):
    ...
    def process_trade(self, symbol, bid_price, ask_price):
        ...
        #long new entry
        #long increase position
        #long exit
        #short increase position
        #short exit
        ...
        if position.qty_available > 0 and bid_price >= position.entry_price + target_price:
    ...
    def stop_trading(self):
        ...
        for symbol in self.stocks:
            try:
                position = self.positions[symbol]
                order = self.trading_client.submit_order(LimitOrderRequest(symbol=symbol, qty=position.qty_available, side='sell',limit_price = round(position.entry_price + target_price,2), time_in_force='day', extended_hours=True))
    ...
    def start_trading(self):
        ...
        for symbol in self.stocks:
            ...
            try:
                ...
                position.calc_prices()
                position.qty_available = abs(int(acct_position.qty))
                ...
                print(f'Existing position: {symbol} {position.side} {position.qty} {position.last_price}')
            except Exception as e:
        ...
        async def handle_trade_updates(data):
            ...
            if data.event == 'fill':
                ...
                try:
                    acct_position = self.trading_client.get_open_position(symbol)
                    print('acct_position--', acct_position)
                    position.entry_price = float(acct_position.avg_entry_price)
                    position.qty_available = int(acct_position.qty)
                    position.qty = abs(position.qty_available) if position.qty_available != 0 else initial_qty
                    position.side = acct_position.side
                except Exception as e:
                    position.entry_price = 0
                    position.qty_available = 0
