...
class BuytheDip(object):
    ...
    def process_current_tick(self, tick_open, tick_close):
        try:
            ...
            if self.open_pl > 0:
                self.exit_qty = abs(self.open_pl / self.tgt)
            else:
                self.exit_qty = 0;
        except:
            self.open_qty = 0
        ...
        if int(self.open_qty) < 0 and float(self.open_pl) / abs(int(self.open_qty)) > self.tgt:
            self.current_order = self.api.submit_order(
                    self.symbol, abs(int(self.open_qty)), 'buy',
            ...
            try:
                ...
                self.current_position = 0
            print(f'1. buying at {tick_close}, equity={self.api.get_account().equity}, current position: {self.current_position}')
        ...
        if int(self.open_qty) > 0 and float(self.open_pl) / abs(int(self.open_qty)) > self.tgt:
            self.current_order = self.api.submit_order(
                    self.symbol, self.open_qty, 'sell',
                    'market', 'day', None, None, None, False
                )
            try:
                self.current_position = self.api.get_position(self.symbol).qty
                ...
                self.current_position = 0
            print(f'1. selling at {tick_close}, equity={self.api.get_account().equity}, current position: {self.current_position}')
            self.last_price = tick_close
