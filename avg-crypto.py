...
class My(object):
    def __init__(self):
        ...
        self.ws = CryptoDataStream(self.key_id, self.secret_key, url_override="wss://stream.data.alpaca.markets/v1beta1/crypto")
        self.ts = TradingStream(self.key_id, self.secret_key, paper = not self.live)

    def check_bp_and_order(self, symbol, price, qty, side):
        if float(self.trading_client.get_account().regt_buying_power) < price * qty:
    ...
    def process_trade(self, symbol, bid_price, ask_price):
        ...
        if position.qty_available < initial_qty or position.side == '': #first trade
            print(f'first entry')
        ...
        if position.side == PositionSide.LONG:
            if ask_price < last_price - self.target_price - diff: #long next entry
                print(f'long next entry')
                position.last_price = ask_price
                self.check_bp_and_order(symbol, ask_price, position.qty, OrderSide.BUY)
            elif bid_price > position.entry_price + self.target_price: #long target exit
                print(f'long target exit')
                self.last_order_time = datetime.now()
            ...
            if bid_price > last_price + self.target_price + diff: #short next entry
                print(f'short next entry')
                position.last_price = bid_price
                self.check_bp_and_order(symbol, bid_price, position.qty, OrderSide.SELL)
            elif ask_price < position.entry_price - self.target_price: #short target exit
                print(f'short target exit')
    ...
    def start_trading(self):
        ...
        for symbol in self.stocks:
            ...
            try:
                ...
                print(f'Existing position: {symbol} {position.qty_available} {position.last_price}')
            except Exception as e:
        ...
        async def handle_trade_updates(data):
            if data.event == 'fill' or data.event == 'partial_fill':
                ...
                try:
                    ...
                    position.qty_available = 0
                    position.qty = initial_qty
                    position.side = OrderSide.BUY if position.side == '' else position.side
                    position.last_price = 0
