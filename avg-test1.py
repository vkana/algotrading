...
class My(object):
    ...
    def process_trade(self, symbol, bid_price, ask_price):
        ...
        if position.qty_available > 0 and bid_price >= position.entry_price + self.target_price - 0.02:
    ...
    def start_trading(self):
        ...
        async def handle_trade_updates(data):
            if data.event == 'fill' or data.event == 'partial_fill':
                symbol, side, price = data.order.symbol, data.order.side, data.price
                account = self.trading_client.get_account()
                ...
                try:
                    ...
                    if data.event == 'fill' and data.order.side == 'buy':
                        #print(data)
                        print('here1')
                        position.entries.append({'qty': int(data.qty), 'price': price})
                        # exit last 2 positions avg price + target
                        cost_basis = 0.0
                        exit_qty = 0.0
                        print('here2')
                        for p in position.entries[:2]:
                            ...
                            exit_qty += p.qty
                        if exit_qty > 0:
                            target_price  = round(cost_basis / exit_qty + self.target_price, 2)
                            print(f'{exit_qty} {target_price}')
                            print('here3')
                            if position.exit_order_id != '':
                                try:
                                    self.trading_client.cancel_order_by_id(position.exit_order_id)
                                except:
                                    pass
                                
                                exit_order = None
                                try:
                                    exit_order = self.trading_client.submit_order(LimitOrderRequest(symbol=symbol, side='sell', qty=exit_qty, limit_price=target_price, time_in_force='day', extended_hours=True ))
                                except Exception as e:
                                    print(symbol, e)
                            position.exit_order_id = exit_order.id
                        
                        print('entries after buy:', position.entries)
                    
                    if data.event=='fill' and data.order.side == 'sell' and data.order.id == position.exit_order_id:
                        try:
                            position.entries.pop()
                            position.entries.pop()
                        except:
                            pass
                        print('entries after sell:', position.entries)
